...
if (!empty($_REQUEST) && is_numeric($phoneCheck)) {
    ...
    if (!empty($_REQUEST["vacancy-name"])) {
        $FORM_ID = 5;
        $FORM_EVENT = 'VACANCY';
        $PROP = [];
        $PROP['form_text_14'] = $_REQUEST['name'];
        $PROP['form_text_15'] = $_REQUEST['phone'];
        ...
        $FIELD_PROP['VACANCY_NAME'] = $_REQUEST['vacancy-name'];

        $message .= 'Имя: ' . $_REQUEST['name'] . '<br>';
        $message .= 'Телефон: ' . $_REQUEST['phone'] . '<br><br>';
        $message .= 'Вакансия: ' .$_REQUEST['vacancy-name']. '<br><br>';
