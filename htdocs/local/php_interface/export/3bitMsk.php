...
$xmlUrl = "https://towergroup.ru/3bita/data_msk.xml";
$loadFileEncode = file_get_contents($xmlUrl);
$arDataFile = XML2Array::createArray($loadFileEncode);

/**
 * Застройщики: получение, обновлени и добавление!
 */
...
$arFilter = ['IBLOCK_ID' => REGION_IBLOCK_ID, 'DEPTH_LEVEL' => 2, "IBLOCK_SECTION_ID" => 2];
$arSelect = ['UF_ID'];
...
foreach ($arDataFile['Ads']['Subways']['Subway'] as $subway) {
    if (empty($arMetro[$subway['@attributes']['id']])) {
        $data = [
            "UF_ID" => $subway['@attributes']['id'],
...
$arFilter = ['IBLOCK_ID' => NEW_BUILD_IBLOCK_ID, 'DEPTH_LEVEL' => 1];
$arSelect = ['UF_ID', 'UF_NO_UPDATE', 'UF_NO_UPDATE_DESCRIPTION'];
$rsSect = CIBlockSection::GetList(false, $arFilter, false, $arSelect);
while ($arSect = $rsSect->GetNext()) {
    if (!empty($arSect['UF_ID'])) {
...
foreach ($arDataFile['Ads']['ComplexSubways']['ComplexSubway'] as $complexSubway) {
    $arMetroId[$complexSubway['@attributes']['complexid']][] = $arMetro[$complexSubway['@attributes']['subwayid']];
}
...
foreach ($arDataFile['Ads']['Complexes']['Complex'] as $complex) {
    ...
    if (!empty($arComplexs[$complex['@attributes']['id']])) { // Обновление
        ...
        $section->Update($arComplexs[$complex['@attributes']['id']], $arFields);
    } else {
        $section = new CIBlockSection;
...
foreach ($arDataFile['Ads']['ApartmentsFirst']['Apartment'] as $apartment) {
    ...
    if (!empty($arFlats[$apartment['@attributes']['id']])) {
        ...
        if ($apartment['@attributes']['roomtypeid'] == 0) {
            ...
            $rooms = 9;
        } elseif ($apartment['@attributes']['roomtypeid'] == 33) {
            $rooms = 9;
        }
