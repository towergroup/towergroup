...
class SearchApartmentsComponent extends CBitrixComponent
{
    ...
    protected function exec()
    {
        ...
        if (!$rsCurCity) {
            LocalRedirect('/moskva/nedvizhimost-za-rubezhom/', false, '301 Moved permanently');
        }
        ...
        if (!empty($cityCode) && !empty($objectCode)) { // Страница объекта
            ...
            if (!empty($arFlat)) {
                if($arFlat["ACTIVE"] == 'N'){
                    LocalRedirect('/'.$cityCode.'/nedvizhimost-za-rubezhom/', false, '301 Moved permanently');
                }
                //находим Отделки
                ...
                if (!empty($arFlat["PROPERTY_DECORATION_VALUE"])) {
                    /*$arDecorationValue = CIBlockPropertyEnum::GetList(array(), array("ID" => $arFlat["PROPERTY_DECRATION_VALUE"]));
                    if($decorationValue = $arDecorationValue->GetNext()){
                       $arFlat["PROPERTY_DECORATION_VALUE"] = $decorationValue["VALUE"];
                    }*/
                ...
                if (!empty($arFlat["PROPERTY_REGION_VALUE"])) {
                    ...
                    $arFilter = ['IBLOCK_ID' => REGION_OVERSEAS_IBLOCK_ID, 'DEPTH_LEVEL' => 2, "ID" => $arFlat["PROPERTY_REGION_VALUE"]];
                ...
                if (!empty($arFlat["PROPERTY_COUNTRY_VALUE"])) {

                    $arFilter = ['IBLOCK_ID' => REGION_OVERSEAS_IBLOCK_ID, 'DEPTH_LEVEL' => 1, "ID" => $arFlat["PROPERTY_COUNTRY_VALUE"]];
                    $arSelect = ['NAME', 'UF_ID'];
                    $arSect = CIBlockSection::GetList(false, $arFilter, false, $arSelect)->Fetch();

                    $arFlat["PROPERTY_COUNTRY_VALUE"] = $arSect["NAME"];
                }
                ...
                if (!empty($arFlat["PROPERTY_PHOTOS_VALUE"])) {
                    foreach ($arFlat["PROPERTY_PHOTOS_VALUE"] as $photoId){
                        ...
                        $arFlat["PROPERTY_SLIDER_MINI_VALUE"][] = CFile::ResizeImageGet($photoId, array('width'=>128, 'height'=>128), BX_RESIZE_IMAGE_PROPORTIONAL, true, false, false, 70)["src"];
                    }
                }
                ...
                if (!empty($arObject['FLAT']['PROPERTY_SIMILAR_FLATS_VALUE'])) {
                    ...
                    while ($rsSimilarObject = $rsSimilarObjects->GetNext()){
                        ...
                        $rsSimilarObject["SECTION_PAGE_URL"] = "/nedvizhimost-za-rubezhom/" . $rsSimilarObject['CODE'] . "/";
...
//            $sortBy   = ($_GET['sortBy'] && $_GET['sortBy'] != 'default')? $_GET['sortBy'] : $this->arParams["SORT_BY_DEFAULT"];
            ...
            $cityobject = ($_GET['city']) ? $_GET['city'] : null ;
            ...
            if ($cacheObjectsParamsDefault->initCache(86400, "cache_data_objects_default_params_overseas")) { // проверяем кеш и задаём настройки
            ...
            elseif ($cacheObjectsParamsDefault->startDataCache()) {
                ...
                while ($arObjectApartmentNoParams = $rsObjectApartmentNoParams->Fetch()) {
                    ...
                    if (!in_array($arObjectApartmentNoParams["PROPERTY_CITY_VALUE"],$arObjectsParamsDefault["CITIES"]))
                        $arObjectsParamsDefault["CITIES"][] = $arObjectApartmentNoParams["PROPERTY_CITY_VALUE"];
                }
            ...
            $arCities = $arObjectsParamsDefault["CITIES"];
            ...
            if ($search != null) {
                ...
                foreach ($decodeSearch as $arSearchTag) {
                    $country = str_replace("country_","", $arSearchTag["id"], $areaCountry);
                    ...
                    $city = str_replace("city_","", $arSearchTag["id"], $cityCount);

                    if ($areaCountry > 0) {
                        if ($arSearchTag["exclude"]) {
                            $reqnotCountry[] = $country;
                        }
                        else {
                            $reqCountry[] = $country;
                        }
                ...
                $arFilterApartments = [];
                if(count($reqnotCountry) > 0) {
                    $arFilterApartments["!COUNTRY"] = array_merge(array("LOGIC" => "OR"), $reqnotCountry);
                }
                if(count($reqCountry) > 0) {
                    $arFilterApartments["COUNTRY"] = array_merge(array("LOGIC" => "OR"), $reqCountry);
                    $arFilterApartments["!COUNTRY"] = false;
                }
                ...
                if(count($reqnotCity) > 0) {
                    $arFilterApartments["!CITY"] = array_merge(array("LOGIC" => "OR"), $reqnotCity);
                }
                if(count($reqCity) > 0) {
                    $arFilterApartments["CITY"] = array_merge(array("LOGIC" => "OR"), $reqCity);
                    $arFilterApartments["!CITY"] = false;
                }
            ...
            if($features != null) {
                ...
                if(count($reqFeatures) > 0) {
                    foreach ($reqFeatures as $option) {
                        switch ($option) {
                            case 'terrace':
                                $arFilterApartments["TERRACE"] = 53;
                                break;
                            case 'twotier':
                                $arFilterApartments["TWO_TIER"] = $cityCode == 47;
                                break;
                            case 'highceilings':
                                $arFilterApartments["HIGH_CEILINGS"] = 46;
                                break;
                        }
                    }
            ...
            if($cityobject != null) {
                $isApartmentRequest == false ? $isApartmentRequest = true : null;
                $reqCityObject = (is_array($cityobject)) ? $city : explode(';', $cityobject);
                if(count($reqCityObject) > 0) {
                    $arFilterApartments["CITY"] = array_merge(array("LOGIC" => "OR"), $reqCityObject);
                    $arFilterValues["CITY"] = implode(";",$reqCityObject);
                }
            }
            ...
            /*if ($arFilterApartments) $arFilterObjects["PROPERTY"] = $arFilterApartments;
            ...
            while ($arObjectApartment = $rsObjectApartment->Fetch()) {
                $arObjectApartment["SECTION_PAGE_URL"] = "/".$cityCode."/nedvizhimost-za-rubezhom/" . $arObjectApartment['CODE'] . "/";
