...
<form method="post" class="hardkod-turboyandex" action="<?=sprintf('%s?mid=%s&lang=%s', $request->getRequestedPage(), urlencode($mid), LANGUAGE_ID)?>">
	...
	<tr>
		<td width="40%" class="adm-detail-content-cell-l">
			<label for="export_author"><?=Loc::getMessage("HARDKOD_TURBOYANDEX_EXPORT_AUTHOR") ?></label>
		</td>
		<td width="60%">
			<input type="checkbox"
				id="export_author"
				name="export_author"
				value="1"
				<?=((Option::get(ADMIN_MODULE_NAME, "export_author")) ? " checked='checked'" : "");?>
			/>
		</td>
	</tr>
